#!/usr/bin/env python3
"""
Train gait-based Parkinson's Disease detection model.
"""
...
def load_data():
    """Load gait dataset."""
    data_path = Path('data/raw/gait/gait_data.csv')
    
    if not data_path.exists():
        raise FileNotFoundError(
            f"Gait dataset not found at {data_path}. "
...
def train_model(X_train, y_train, X_val, y_val):
    """Train and optimize gait model."""
    print_section("TRAINING GAIT MODEL")
...
def evaluate_model(model, X_test, y_test, model_name):
    ...
    plt.title(f'Gait Model - Confusion Matrix\n{model_name}')
    plt.ylabel('True Label')
    plt.xlabel('Predicted Label')
    plt.tight_layout()
    plt.savefig(models_dir / 'gait_confusion_matrix.png', dpi=150)
    plt.close()
    ...
    plt.title('Gait Model - ROC Curve')
    plt.legend()
    plt.grid(alpha=0.3)
    plt.tight_layout()
    plt.savefig(models_dir / 'gait_roc_curve.png', dpi=150)
    plt.close()
...
def main():
    print_section("GAIT-BASED PD DETECTION MODEL TRAINING")
    
    # Load data
    print("\n1. Loading gait dataset...")
    ...
    model_path = models_dir / 'gait_model.joblib'
    scaler_path = models_dir / 'gait_scaler.joblib'
